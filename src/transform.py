...
def dim_currency(currency_df):
    ...
    try:
        ...
        c = CurrencyCodes()
        dim_currency_df['currency_name'] = dim_currency_df["currency_code"].apply(c.get_currency_name)
...
def dim_staff(staff_df):
    ...
    try:       
        ...
        staff_department_df = pd.merge(staff_df,department_df[['department_id','location', 'department_name']],
                                       on='department_id', how='left')
        dim_staff_df = pd.DataFrame()
        dim_staff_df = staff_department_df[["staff_id", "first_name", "last_name", "department_name", 
...
def dim_counterparty(counterparty_df):
    ...
    try: 
        ...
        counterparty_address_df = pd.merge(counterparty_df,address_df,
                                        left_on='legal_address_id', right_on='address_id', how='left')
        dim_counterparty_df = pd.DataFrame()
        dim_counterparty_df = counterparty_address_df[["counterparty_id", "counterparty_legal_name", "address_line_1", "address_line_2", 
...
def dim_location(address_df):
    ...
    '''
    try: 
        dim_location_df = address_df[["address_id", "address_line_1", "address_line_2", "district", 
...
def lambda_handler(event=None, context=None):   
    ...
    try: 
        ...
        if bucket_objects["KeyCount"] == 0:
            ...
            file_path_list.append(write_parquet_to_s3_bucket(fact_sales_order(sales_order_df), "fact_sales_order"))

            # creation of dimension location 
            location_files = get_all_files("address")
            location_df = read_parquet_from_s3(location_files)
            file_path_list.append(write_parquet_to_s3_bucket(dim_location(location_df), "dim_location"))
            ...
            file_path_list.append(write_parquet_to_s3_bucket(dim_counterparty(counterparty_df), "dim_counterparty"))

            # creation of dimension staff 
            staff_files = get_all_files("staff")
            staff_df = read_parquet_from_s3(staff_files)
            file_path_list.append(write_parquet_to_s3_bucket(dim_staff(staff_df), "dim_staff"))

            # creation of dimension currency 
            currency_files = get_all_files("currency")
            currency_df = read_parquet_from_s3(currency_files)
            file_path_list.append(write_parquet_to_s3_bucket(dim_currency(currency_df), "dim_currency"))

            # creation of dimension design 
            design_files = get_all_files("design")
            design_df = read_parquet_from_s3(design_files)
            file_path_list.append(write_parquet_to_s3_bucket(dim_design(design_df), "dim_design"))
