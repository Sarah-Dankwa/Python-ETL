...
resource "aws_cloudwatch_metric_alarm" "extract_metric_alarm" {
  ...
  alarm_actions = [ "arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${data.aws_sns_topic.step_functions_workflow_sns.arn}", "arn:aws:sns:eu-west-2:590183674561:user-updates-topic"]
