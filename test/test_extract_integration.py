...
class TestFetchFromDB:
    ...
    def test_logs_error_if_too_few_tables(
        ...
        with caplog.at_level(logging.ERROR):
            fetch_from_db()
            assert "There are 10 tables fetched from the database - please check." in caplog.text
...
class TestLambdaHandler:
    ...
    @pytest.mark.it(
            "when bucket is not empty retrieve datetime parameter is called"
    )
    @patch("src.extract.BUCKET_NAME", "test-ingestion-bucket")
    @patch(
        "src.extract.retrieve_datetime_parameter", return_value="2024-01-01"
    )
    ...
    @pytest.mark.it(
        "when bucket is empty lambda handler calls full " 
